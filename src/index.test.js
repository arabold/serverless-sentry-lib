	...
	describe("No Sentry installed", () => {
		...
		describe("Async/Await (Promises)", () => {
			it("should return fulfilled Promise", () => {
				...
				const wrappedHandler = SentryLambdaWrapper.handler(SentryMock, handler);
				return expect(
					wrappedHandler(mockEvent, mockContext, sinon.stub())
	...
	describe("Sentry installed", () => {
		...
		describe("Context", () => {
			it("should retain original context object", () => {
				...
				const wrappedHandler = SentryLambdaWrapper.handler(SentryMock, handler);
				return expect(wrappedHandler(mockEvent, mockContext)).to.eventually.be
					.fulfilled;
			});
		});
		...
		describe("Settings", () => {
			...
			describe("autoBreadcrumbs", () => {
				it("should trace Lambda function as breadcrumb", done => {
					...
					const wrappedHandler = SentryLambdaWrapper.handler(
						SentryMock,
						handler
					);
					const callback = (err, result) => {
			...
			describe("captureTimeoutWarnings", function() {
				...
				const handler = (event, context) => {
					...
					return new Promise(resolve =>
						setTimeout(resolve, remainingTime + 100)
					);
				};
				...
				it("should warn if more than half of the originally available time has passed", () => {
					...
					return expect(
						...
						expect(spyScopeSetExtras.firstCall.args[0].TimeRemainingInMsec)
							.to.be.lessThan(remainingTime / 2)
				...
				it("should error if Lambda timeout is hit", function() {
					...
					return expect(
						...
						expect(spyScopeSetExtras.secondCall.args[0].TimeRemainingInMsec)
							.to.be.lessThan(501)
							.and.above(400);
					});
				});
